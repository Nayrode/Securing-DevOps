...
* **Scan Trivi** : Analyse ciblée des configurations de l’infrastructure, notamment du service SMTP, qui est un vecteur critique dans le contexte d’une application de messagerie.
...
2. **Mettre à jour les dépendances critiques** :
   ...
   * Corriger toutes les dépendances listées par Snyk et Trivi.
...
3. **Documenter et former l’équipe** :
   Rédiger des bonnes pratiques de sécurité, faire des formations internes sur la gestion des secrets et la configuration sécurisée des services.
